...
| Position Paper QA grün | `PASS` | Position paper (`D5`): accepted candidate `d5-i2` at iteration `2`, `14` of `14` criteria PASS, receipt `d5-i2-a1-AC01` exit `0`. Citation-and-numbers audit (`D6`): accepted candidate `d6-i2` at iteration `2`, `10` of `10` criteria PASS, receipt `d6-i2-a1-K1` exit `0`. Both from the internal *Abschlussbericht* Section B's run-trace table. |
