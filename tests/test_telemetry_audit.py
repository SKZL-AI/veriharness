...
def _load():
    spec = importlib.util.spec_from_file_location(
        "telemetry_audit", ROOT / "tools" / "telemetry_audit.py"
    )
...
def _fields(sentences: list[dict]) -> dict:
    return ta.check(sentences)
...
def test_tokens_the_harness_never_reported_are_not_available_not_missing():
    f = _fields([sentence(tokens_in=None, tokens_out=None)])
...
def test_a_dispatch_that_needed_no_retry_is_not_a_missing_measurement():
    f = _fields([sentence(retries=0, waited_seconds=0)])
...
def test_zero_receipts_on_the_verifying_dispatch_is_a_gap():
    ...
    f = _fields([sentence(role="qa", receipts=0)])
...
def test_zero_discriminating_criteria_is_a_real_answer_and_not_a_gap():
    ...
    """
    f = _fields([sentence(role="qa", discriminating=0, artefactual=0)])
...
def test_a_record_that_predates_the_witness_fields_is_unknown_not_missing():
    ...
    """
    f = _fields([sentence(witnessed_trees=None, witnessed_listings=None)])
...
def test_a_condition_may_not_exempt_a_field_by_asking_about_itself():
    ...
    for field_, condition_ in ta.CONDITIONAL.items():
        source = inspect.getsource(condition_)
        assert f'r.get("{field_}")' not in source, (
            f"{field_} exempts itself exactly when it is empty")
...
def test_a_campaign_with_every_shape_covers_them():
    sentences = [
        sentence(role="planner"),
        sentence(role="developer"),
        sentence(role="qa", receipts=2, discriminating=1),
        sentence(role="planner", outcome="failed", detail="x",
             failure_class="CONTRACT_INVALID", retries=1, waited_seconds=5),
    ]
    a = ta.coverage_(sentences)
    assert all(a.values()), [k for k, v in a.items() if not v]


def test_an_unlabelled_proxy_is_reported():
    a = ta.coverage_([sentence(quota_proxy=3.0, quota_proxy_kind="")])
...
def test_an_unknown_token_count_does_not_become_a_zero_in_the_total():
    k = ta.aggregation_control([sentence(tokens_in=None, tokens_out=None),
                                  sentence(tokens_in=10, tokens_out=5)])
...
def test_a_complete_set_of_records_does_total():
    k = ta.aggregation_control([sentence(tokens_in=10, tokens_out=5)])
...
def test_the_control_reports_how_many_inputs_it_could_not_use():
    k = ta.aggregation_control([sentence(tokens_in=None, tokens_out=None)] * 3)
...
@pytest.fixture
def log_(tmp_path: Path) -> Path:
    run = tmp_path / "r1"
    run.mkdir()
    return run.parent


def _write_file(root: Path, sentences: list[dict]) -> None:
    (root / "r1" / "telemetry.jsonl").write_text(
        "".join(json.dumps(s) + "\n" for s in sentences))


def test_a_fully_wired_log_validates(log_):
    _write_file(log_, [
        sentence(role="planner"),
        sentence(role="developer"),
        sentence(role="qa", receipts=2, discriminating=1),
    ...
    assert ta.main(["--run-root", str(log_), "--run-id", "r1"]) == 0


def test_a_log_of_well_typed_blanks_does_not_validate(log_, capsys):
    _write_file(log_, [
        sentence(role="planner", provider="", model=""),
        sentence(role="developer", provider="", model=""),
        sentence(role="qa", provider="", model="", receipts=0),
    ])
    assert ta.main(["--run-root", str(log_), "--run-id", "r1"]) == 1
...
def test_receipts_summed_must_agree_with_the_receipt_files(tmp_path):
    ...
    for name in ("K1", "K1-basis"):
        (run / "receipts" / f"r1-i1-a1-{name}.json").write_text("{}")

    k = ta.aggregation_control([sentence(role="qa", receipts=0)], run)
    assert k["receipt_files_on_disk"] == 2
    assert k["receipts_summed"] == 0
    assert k["receipts_agree_with_the_files"] is False

    k = ta.aggregation_control([sentence(role="qa", receipts=2)], run)
...
def test_without_a_run_directory_the_receipt_check_says_it_did_not_run(tmp_path):
    ...
    k = ta.aggregation_control([sentence(role="qa", receipts=2)], None)
...
def test_an_uneventful_campaign_is_not_the_same_as_a_defective_one():
    ...
    """
    run = pathlib.Path(tempfile.mkdtemp()) / "r1"
    (run / "receipts").mkdir(parents=True)
    (run / "receipts" / "r1-i1-a1-K1.json").write_text("{}")
    (run / "telemetry.jsonl").write_text("".join(
        json.dumps(sentence(role=r, receipts=1 if r == "qa" else 0)) + "\n"
        for r in ("planner", "developer", "qa")))

    assert ta.main(["--run-root", str(run.parent), "--run-id", "r1"]) == 1
    report = json.loads(_json_report(run.parent, "r1"))
    assert report["fields_validated_on_real_dispatches"] == "yes"
    assert report["telemetry_validated_on_real_dispatches"] == "no"
    assert report["coverage_gaps"] == []
    assert set(report["shapes_not_observed"]) == {
...
def test_a_real_field_gap_is_not_excused_by_an_uneventful_campaign():
    ...
    (run / "receipts" / "r1-i1-a1-K1.json").write_text("{}")
    (run / "telemetry.jsonl").write_text("".join(
        json.dumps(sentence(role=r, provider="", receipts=1 if r == "qa" else 0)) + "\n"
        for r in ("planner", "developer", "qa")))

    report = json.loads(_json_report(run.parent, "r1"))
    assert report["fields_validated_on_real_dispatches"] == "no"
    assert "provider" in report["fields_with_gaps"]


def _json_report(root, run_id) -> str:
    ...
    with redirect_stdout(buffer_):
        ta.main(["--run-root", str(root), "--run-id", run_id, "--json"])
...
def test_a_refused_dispatch_may_report_zero_provider_calls():
    ...
    """
    assert ta._state("provider_calls", 0,
                       _sentence(outcome="failed", provider_calls=0)) == ta.MEASURED
...
def test_a_successful_dispatch_reporting_zero_provider_calls_is_still_a_gap():
    ...
    assert ta._state("provider_calls", 0,
                       _sentence(outcome="ok", provider_calls=0)) == ta.MISSING
...
def test_a_refused_dispatch_is_not_required_to_carry_a_witness():
    ...
    a = ta.coverage_([_sentence(), _sentence(outcome="failed", provider_calls=0,
...
def test_a_dispatch_that_ran_without_a_witness_is_still_a_coverage_gap():
    ...
    a = ta.coverage_([_sentence(), _sentence(provider_calls=1, witnessed_trees=None)])
...
def test_a_record_predating_provider_calls_is_still_asked_for_its_witness():
    ...
    a = ta.coverage_([_sentence(), _sentence(provider_calls=None, witnessed_trees=None)])
    assert a["witness_coverage_recorded"] is False
